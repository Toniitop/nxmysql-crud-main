...
export async function newProveedor(formData) {
  try {
    ...
    const query = 'insert into proveedores(codigo,nombre,telefono) values (?, ?, ?)';
    const results = await db.query(query, [codigo, nombre, telefono]);
    console.log(results);
  } catch (error) {
    console.log(error);
  }
...
export async function editProveedor(formData) {
  ...
  try {
    ...
    const results = await db.query(query, [{codigo, nombre, telefono}, codigo]);
    console.log(results);
  } catch (error) {
    console.log(error);
  }
